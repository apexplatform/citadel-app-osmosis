import { Button, Spinner } from '@citadeldao/apps-ui-kit/dist/main';
import { useSelector } from 'react-redux';
import { swapActions } from '../../store/actions';
const SwapButton = (props) => {
    const { swapStatus, tokenIn, disableSwap, outAmout, amount  } = useSelector((state) => state.swap)
    const customStyle = {
        width: 'auto',
        marginTop: '20px',
    }
	return (
        <div className='center'>
            {+amount !== 0 && outAmout === 0 ? <Button disabled style={customStyle}><Spinner/></Button> : 
            <div>
